...
func _ready() -> void:
	...
	var room_1: Node = get_tree().current_scene.get_node("Fortress Storage")
	var room_2: Node = get_tree().current_scene.get_node("Fortress Hall Node")
	var gate_1: Node = get_tree().current_scene.get_node("Fortress Storage East Gate")
	var gate_2: Node = get_tree().current_scene.get_node("Fortress Hall Gate")
	...
	if room_1 != null and room_2 != null:
		...
		Global._notify("Fortress Storage Gate is open!", 2.0)
		await timer.timeout
		
		timer = get_tree().create_timer(3)
		cutscene_camera.global_position = room_2.global_position
		Global._notify("Fortress Hall Gate is open!", 2.0)
		await timer.timeout
